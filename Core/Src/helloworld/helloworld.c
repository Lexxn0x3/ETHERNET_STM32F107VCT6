...
#define MAX_NAME_SIZE 32
 ...
 uint16_t LEDPins[] = {GPIO_PIN_0, GPIO_PIN_1, GPIO_PIN_2, GPIO_PIN_3, GPIO_PIN_4, GPIO_PIN_5, GPIO_PIN_6, GPIO_PIN_7, GPIO_PIN_8, GPIO_PIN_9, GPIO_PIN_10, GPIO_PIN_11, GPIO_PIN_12, GPIO_PIN_13, GPIO_PIN_14, GPIO_PIN_15};
...
static err_t HelloWorld_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err)
{
  ...
  {        
    ...
    if(done) 
    {
      ...
      if (!(name->bytes[0] == 'L' && name->bytes[1] == 'E' && name->bytes[2] == 'D')) {
        tcp_write(pcb, HELLO, strlen(HELLO), 1);

        tcp_write(pcb, name->bytes, name->length, TCP_WRITE_FLAG_COPY);
      } else {
        tcp_write(pcb, LEDTEXT, strlen(LEDTEXT), 1);

        tcp_write(pcb, name->bytes, name->length, TCP_WRITE_FLAG_COPY);

        uint16_t pin = 0;

        if (name->bytes[4] == '\r' || name->bytes[4] == '\n') {
          pin = name->bytes[3] - '0';
        } else {
          pin = (name->bytes[3] - '0') * 10 + (name->bytes[4] - '0');
        }

        if (0 <= pin && pin <= 15) {
          HAL_GPIO_TogglePin(GPIOD, LEDPins[pin]);
        }
      }
      name->length = 0;
    }
